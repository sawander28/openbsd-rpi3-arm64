...
def change_mac_address(interface_name, new_mac):
    ...
    try:
        ...
        # Extract just the interface name (e.g., "Ethernet 4" from "Ethernet 4 ASIX USB to Gig 9A-9A-1F-2F-CC-CC")
        # Split by space and take the first two parts to get "Ethernet 4"
        name_parts = interface_name.split()
        if len(name_parts) >= 2:
            target_interface = f"{name_parts[0]} {name_parts[1]}"
        else:
            target_interface = name_parts[0]
        
        print(f"Target interface: {target_interface}")
        
        # Use exact match for the interface name
        ps_find_cmd = f'powershell -Command "Get-NetAdapter | Where-Object {{ $_.Name -eq \'{target_interface}\' }} | Format-List -Property InterfaceDescription, Name, DeviceID, DriverFileName, DriverVersion"'
        ...
        if not adapter_name:
            print(f"Error: Could not find adapter name for {target_interface}")
            return False
            
        print(f"Found adapter name: {adapter_name}")
        
        # Verify we have the correct adapter
        if adapter_name != target_interface:
            print(f"Warning: Expected {target_interface} but found {adapter_name}")
        
        # Find the correct registry key by searching for the exact interface description
        # First, get the interface description for our target adapter
        interface_desc = None
        for line in adapter_info.splitlines():
            if line.strip().startswith("InterfaceDescription"):
                interface_desc = line.split(":", 1)[1].strip()
                break
        
        if not interface_desc:
            print(f"Error: Could not find interface description for {target_interface}")
            return False
            
        print(f"Interface description: {interface_desc}")
        ...
        # Find the matching registry key by interface description
        registry_key = None
        registry_sections = registry_output.split("HKEY_LOCAL_MACHINE")
        
        for section in registry_sections:
            if interface_desc in section:
                registry_key = "HKEY_LOCAL_MACHINE" + section.split("\r\n")[0]
                break
        
        if not registry_key:
            print(f"Error: Could not find registry key for {interface_desc}")
            # Fallback method - search through numbered registry keys
            print("Trying alternative registry identification method...")
            
            for i in range(50):  # Try first 50 possible indexes
                ...
                try:
                    ...
                    if interface_desc in test_output:
        ...
        if not registry_key:
            print(f"Error: Could not find registry key for {interface_desc} after multiple attempts")
            return False
        ...
        try:
            ...
            print("Could not verify MAC address change")
